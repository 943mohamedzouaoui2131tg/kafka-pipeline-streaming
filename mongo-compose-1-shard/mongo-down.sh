...
echo ""
echo "=== Step 3: Removing Shard from Cluster ==="
if safe_exec "mongos"; then
...
echo ""
echo "=== Step 7: Removing Docker Volumes (Data Cleanup) ==="
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Also try to remove volumes with compose prefix
    docker volume rm mongo_compose_configsvr-data 2>/dev/null || true
    docker volume rm mongo_compose_shard1a-data 2>/dev/null || true
    docker volume rm mongo_compose_shard1b-data 2>/dev/null || true
...
echo ""
echo "=== Step 8: Removing MongoDB Network ==="
if docker network ls | grep -q mongo_compose_default; then
    docker network rm mongo_compose_default 2>/dev/null || true
    echo "✓ Network removed"
elif docker network ls | grep -q mongo-cluster-network; then
